...
)  # Added QSlider, QDoubleSpinBox, QStyledItemDelegate, QStyleOptionViewItem
...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        self.current_rebindings_root: ET.Element | None = (
            None  # To store the loaded XML root
        )
        self.current_rebindings_filepath: str | None = (
            None  # To store the path of the loaded rebindings file
        )
        self.item_id_to_rebind_element: dict[int, ET.Element] = (
            {}
        )  # Maps tree item id to its XML rebind element

        self.current_usersettings_root: ET.Element | None = None
        self.current_usersettings_filepath: str | None = None
        self.item_id_to_usersetting_element: dict[int, ET.Element] = (
            {}
        )  # Maps tree item id to its user setting <Class> element
        ...
        self.status_label = QLabel(
            "Welcome to New World Config Manager!"
        ...
        self.action_status_label = QLabel(
            "Load a configuration file to begin."
        )  # For persistent status like loaded file
        ...
        self.restore_backup_button.setEnabled(
            self.config_parser.new_world_config_dir is not None
        )
        ...
        self.save_button.clicked.connect(
            self.handle_save_current_config
        )  # Changed handler
    ...
    def _populate_rebindings_tree(self, root_element: ET.Element):
        ...
        for actionmap_element in root_element.findall("actionmap"):
            ...
            for action_element in actionmap_element.findall("action"):
                ...
                if (
                    not rebinds
                ):  # Action might not have a rebind, or structure is different
                    ...
                    for rebind_element in rebinds:
                        ...
                        self.item_id_to_rebind_element[id(action_row_item)] = (
                            rebind_element  # Store mapping using item's id
                        )
    ...
    def _populate_generic_xml_tree(self, parent_item_or_tree, element: ET.Element):
        ...
        if element.tag == "Class" and "field" in element.attrib:
            ...
            value = element.get(
                "value", ""
            )  # Default to empty string if no value attribute
            ...
            if is_specific_reticle_color:
                ...
                if parsed_rgba_floats is None:
                    ...
                    print(
                        f"Applied default color {parsed_rgba_floats} to '{field_name}' due to missing/invalid value: '{value}'."
                    )
            ...
            if use_color_editor_widget:
                ...
                q_color = QColor(
                    int(editor_initial_rgba[0] * 255),
                    int(editor_initial_rgba[1] * 255),
                    int(editor_initial_rgba[2] * 255),
                    int(editor_initial_rgba[3] * 255),
                )
                pixmap_preview.fill(q_color)
                setting_item.setIcon(0, QIcon(pixmap_preview))

                editor_widget = MainWindow.ColorEditorWidget(
                    initial_rgba_floats=editor_initial_rgba
                )
                self.config_tree_widget.setItemWidget(setting_item, 1, editor_widget)
                editor_widget.color_changed_signal.connect(
                    lambda new_rgba, item=setting_item: self.handle_color_editor_changed(
                        item, new_rgba
                    )
                )
            ...
            self.item_id_to_usersetting_element[id(setting_item)] = (
                element  # Map item to its <Class> element
            )
        ...
        item = None
        if (
            element.tag != "Class" or parent_item_or_tree == self.config_tree_widget
        ):  # Show root or non-Class tags
    ...
    def handle_load_rebindings(self, prompt_for_backup=True):
        ...
        if not temp_rebindings_filepath:
            ...
            QMessageBox.information(
                self,
                "Load Rebindings",
                "Could not find rebindings file. Check console for details.",
            )
        ...
        rebindings_data_root = self.config_parser.load_xml_config(
            temp_rebindings_filepath
        )
        ...
        if rebindings_data_root is not None:
            ...
            self.action_status_label.setText(
                f"Rebindings loaded: {Path(self.current_rebindings_filepath).name}"
            )
            ...
            self.status_label.setText(
                f"Failed to load rebindings XML from {Path(temp_rebindings_filepath).name}."
            )
            ...
            QMessageBox.information(
                self,
                "Load Rebindings",
                f"Could not load rebindings from {Path(temp_rebindings_filepath).name}. Check console.",
            )
        ...
        try:
            ...
            if result:
                ...
                self.current_usersettings_filepath = (
                    javsave_path  # Store path even if root is None
                )
                if root_element is not None:
                    ...
                    self.config_tree_widget.setColumnWidth(
                        1, 350
                    )  # Value column (for sliders)
                    ...
                    self.status_label.setText(
                        f"Found {Path(javsave_path).name}, but failed to parse as XML. See console."
                    )
                    self.action_status_label.setText(
                ...
                QMessageBox.information(
                    self,
                    "Load User Settings",
                    "Could not find usersettings.javsave. Check console for details.",
                )
                self.save_button.setEnabled(False)
                self.changes_made_in_current_config = False
                self.reset_changes_button.setEnabled(False)
    ...
    def handle_restore_from_backup(self):
        ...
        selected_backup_path_str = QFileDialog.getExistingDirectory(
            self, "Select Backup Folder to Restore", str(backup_parent_dir)
        )
    ...
    def handle_item_changed(self, item: QTreeWidgetItem, column: int):
        ...
        """
        item_id = id(item)
        if column == 1 and not self.config_tree_widget.itemWidget(
            item, 1
        ):  # Only if no custom widget
            if item_id in self.item_id_to_rebind_element:
                ...
                print(
                    f"Updated rebind action '{action_description}' to '{new_value}' in memory."
                )
                self.changes_made_in_current_config = True
                self.reset_changes_button.setEnabled(True)
                self.status_label.setText(
                    "Changes made. Click 'Save Current Config' or 'Reset Current Changes'."
                )
                ...
                usersetting_element.set(
                    "value", new_value_text.strip()
                )  # Update the 'value' attribute
                field_name = usersetting_element.get("field")
                print(
                    f"Updated user setting '{field_name}' to '{new_value_text.strip()}' in memory."
                )
                self.changes_made_in_current_config = True
                self.reset_changes_button.setEnabled(True)
                self.status_label.setText(
    ...
    def handle_color_editor_changed(
        ...
        if item_id in self.item_id_to_usersetting_element:
            ...
            field_name = usersetting_element.get("field")
            print(
                f"Updated user setting (color) '{field_name}' to '{new_value_str}' in memory via sliders."
            )
            self.changes_made_in_current_config = True
            self.reset_changes_button.setEnabled(True)
            self.status_label.setText(
                "Color changes made. Click 'Save Current Config' or 'Reset Current Changes'."
            )
    ...
    def handle_reset_changes(self):
        if not (self.current_rebindings_filepath or self.current_usersettings_filepath):
            QMessageBox.information(
                self, "Reset Changes", "No configuration is currently loaded to reset."
            )
            return
        ...
        if (
            self.current_rebindings_root is not None
            and self.current_rebindings_filepath is not None
        ):
            self.handle_load_rebindings(prompt_for_backup=False)
        elif (
            self.current_usersettings_root is not None
            and self.current_usersettings_filepath is not None
        ):
            self.handle_load_user_settings(prompt_for_backup=False)
        elif (
            self.current_usersettings_filepath is not None
        ):  # Case: user settings file found but failed to parse
            self.handle_load_user_settings(prompt_for_backup=False)
        else:
            self.status_label.setText(
    ...
    def handle_save_current_config(self):
        if self.current_rebindings_root and self.current_rebindings_filepath:
            success = self.config_parser.save_xml_config(
                self.current_rebindings_filepath, self.current_rebindings_root
            )
            if success:
                QMessageBox.information(
                    self,
                ...
                self.action_status_label.setText(
                    f"Rebindings saved: {Path(self.current_rebindings_filepath).name}"
                )
            ...
            success = self.config_parser.save_xml_config(
                self.current_usersettings_filepath, self.current_usersettings_root
            )
            if success:
                QMessageBox.information(
                    self,
                ...
                self.action_status_label.setText(
                    f"User settings saved: {Path(self.current_usersettings_filepath).name}"
                )
